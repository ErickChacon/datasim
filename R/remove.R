...
# discrimination2 <- matrix(seq(0.4, 1.5, length.out = q), nrow = 1)
# f <- list(
#   prob ~ mfa(ones, beta = get("difficulty")) +
#     re(id, sigma = 1, q = get("q")):mfa(ones, beta = get("discrimination1")) +
#     re(id, sigma = 1, q = get("q")):mfa(ones, beta = get("discrimination2")),
#   size ~ I(1)
#   )
# formula <- f
# idata <- NULL
